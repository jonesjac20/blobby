...
- **`/healthz` never 200:** If curl times out with 0 bytes after a task-definition replace, the smoke was hitting the *previous* revision's ENI (`deployment_maximum_percent = 100` stops the old task first). `preview.yml` waits `services-stable` and `preview_task_url.sh` pins the current task definition. If it still fails: ECS task stopped — CloudWatch `/ecs/blobby-preview` for the Python traceback. SG must allow 8000 from `0.0.0.0/0` (GitHub-hosted curl comes from the internet).
