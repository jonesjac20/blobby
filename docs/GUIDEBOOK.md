...
- [ ] **[Agent]** `client/index.html` — greeting menu over a full-window canvas: name field, color picker, **Play**, **Spectate**. Play sends `join` with the chosen name and color. Spectate never sends `join`; the tab only receives `state`. Markup is in place; `game.js` wiring is the next slice.
