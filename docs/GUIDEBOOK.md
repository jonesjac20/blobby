...
- **Bot brain exceeds source plan §7.** Section 7 is “nearest edible, flee if a larger player is closer.” That loop deadlocks, never catches a competent player, and suicide-splits. Phase 6 follows [`bot-logic.md`](bot-logic.md): four states (Graze / Hunt / Flee / Recover), limited vision, per-piece classification. The file is the spec; `bots/simple_bot.py` / `bots/fleet.py` are the Phase 6 plumbing.
- **Shared bot WebSocket.** `--count N` opens one socket and N `join`s (`bot: true`). The game `_emit`s `state` once to that socket; the fleet parses once and `decide()`s per live id, tagging `input`/`split` with `"id"`. `game_over` carries `"id"` so only that slot respawns. Browsers stay one-socket-one-life (omit `id`; a second `join` while alive is still ignored). `--sockets` restores one WebSocket per bot for posterity. A remote bots host is optional and does not replace the shared socket.
...
- **Greeting menu, color, and death.** Source section 4 has `join` / `input` / `split` and a `state` broadcast of `{id, name, pieces}`. Connecting here does not spawn. `join` carries a player-chosen `color`; the server replies with `welcome` `{id}` so a client can follow-cam without matching on name. A last-piece eat removes the player from the world and sends `game_over` `{id, peak_mass, survival_seconds}` on that socket only (`id` names which life died — browsers ignore it; a fleet socket needs it). The next `join` on the same socket is a respawn. A bot socket (`"bot": true`) may `join` again while other owned lives still exist. Spectating is connecting and never sending `join`. Phase 3 owns the menu and Game Over UI; Phase 2 owns the protocol so that UI can exist. `state` players also carry `color`. `input` / `split` accept optional `"id"`; required when the socket owns more than one life.
...
- [x] **[Agent]** `server/config.py`.
  - **From source plan section 5:** `TICK_RATE = 30`, `MIN_SPLIT_MASS = 35`, `MAX_PIECES = 8`, `EAT_RATIO = 1.25`. Live `SPLIT_KICK_DECAY_SECONDS = 3` (plan said ~0.5s). Plus `REMERGE_SECONDS = 10`, our pick from the plan's 10–15s range.
...
- [x] **[Agent]** `bots/simple_bot.py` — WebSocket client using the same `join`/`input`/`split` protocol. CLI args for name, server URL, count. Default `--count N` is one shared socket (`bots.fleet`); `--sockets` is one WebSocket per bot. On disconnect it either reconnects or exits cleanly, per the exit criterion below.
...
- [x] **[Agent]** Run `python -m bots.simple_bot --count 17` against a local server. Confirm the tick loop still holds ~30Hz with that many players in the world (tick count vs wall-clock over ~30s, not a short burst). Server log lines `tick N players=P sockets=S pieces=K hz=H step_ms=S emit_ms=E`. Shared socket should show `sockets=` at 1 plus any human tabs. Measured 2026-08-17: 30 sockets, ~30s, hz 29.5–30.3 (pre-shatter, one-WS-per-bot; lobby is now 17 lives on one socket). Deployed hosts spawn that table themselves: Compose `bots` sidecar on production, Fargate `bots` sidecar on each PR preview (`--count 17`). Do not run this by hand against those URLs unless you want extras. `--sockets` is the posterity load test when you want socket count to match player count.
...
Open `http://localhost:8000`, Play. Watch the server log for `hz=` near 30 with `players=17` and `sockets=` at 1 plus your tab. Ctrl+C on the bot process exits 0. `python -m bots.simple_bot --count 17 --sockets` is the one-WS-per-bot path.
