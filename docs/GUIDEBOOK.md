...
- **Split kick scales with parent radius.** Source section 5 says only that the new piece gets a velocity kick that decays over ~0.5s. A flat speed made large splits a twitch — resting distance grows as `sqrt(mass)` while travel stayed fixed (feel-pass A2). Displacement is now `SPLIT_KICK_RADII` times the pre-split parent piece's radius, capped at `SPLIT_KICK_MAX_ARENA_FRACTION` of the shorter arena axis so a giant cannot lunge across the map. The decay window is `SPLIT_KICK_DECAY_SECONDS` (0.6, near the plan's 0.5s — a 3s coast left the pop feeling floaty). Only the new piece is kicked; the parent stays at half mass with its kick cleared. After the kick, cohesion at `COHESION_SPEED` snaps the halves back into a cluster; a split player also steers every piece at `speed_for_mass(total)` so light fragments cannot string out into a line.
...
- **Mass cap into an inert corpse.** A join with `"bot": true` (the sidecar sends this; the browser menu does not) that reaches `BOT_BURST_MASS` (55000) peels down to `BOT_BURST_REMNANT_MASS` (1000). A human life that reaches `PLAYER_BURST_MASS` (75000) peels down to `PLAYER_BURST_REMNANT_MASS` (1500). Excess becomes a socket-less `inert` player: cannot eat, never remerges, shatters once into `BURST_SHARDS` (6) uneven exploded discs. Shards are **not solid** — live bodies walk through them and eat them at the usual mass ratio — and after `INERT_LIFETIME_SECONDS` (20) leftover mass is sprayed as at most `INERT_EVICT_FOOD_MAX` pellets so a corpse cannot pin the map forever. A global `INERT_PIECE_CAP` (102) still evicts the oldest corpse the same way if the lobby fills up first. `state` carries `inert`. The client paints a pastel high-transparency wash of the parent hex, writes mass on every inert disc, and hides the remerge timer. Feel knobs, like `SPAWN_INVULN_SECONDS`.
...
- **The broadcast is guarded against a join landing mid-tick.** `_emit` gathers per-socket sends, so a `join` handled during that gather still splits the broadcast into sockets served before it and sockets served after. Three messages go wrong in that window, and each is dropped rather than sent: a `game_over` from the previous life, which would arrive after the respawn's `welcome` and end a game that just started; a `state` naming the new player *before* its `welcome`, which hands the client an id it does not yet know is its own; and the pre-join snapshot *after* `welcome`, which omits the player entirely so Phase 3's follow-cam finds nothing to follow. A playing socket simply misses that one frame — the next tick's snapshot is correct for it. Spectators are never withheld anything.
- **`peak_mass` is a life high-water mark.** `update_and_eliminate` runs after `simulation.step`, by which point a player killed this tick has no pieces left to measure, and a player that just burst is already down to remnant mass. `simulation` records `Player.last_total_mass` as a never-decreasing total just before eaten pieces are culled (and again at the burst peel), so mass picked up on the tick that killed you — a pellet, or a fragment of the blob that then ate you — and the mass held at burst still reach Game Over and the HUD **best** chip. `state` players carry `peak_mass`. Current mass on the body label and HUD **mass** chip still drops; best does not.
...
- **Food leaves the `state` broadcast.** Source plan section 4 puts a full food list in every `state`. Measured on a six-player world that was 55,503 of 57,126 bytes (97%), resent ~90% of ticks with a byte-identical 600-element array (median pellet-set churn was 0). A `food` message of `[[x, y], ...]` integer pairs is sent as a full snapshot to a socket at `food_version` 0 (joiners) or more than one version behind (a failed send while the field kept changing). A socket that is exactly one version behind gets a `food_delta` `{add, remove}` instead, so grazers do not re-parse 1600 pellets every eat, and a cached client that still did `food = msg.food || []` on every `type: food` does not wipe the pellets. Both strings are dumped once (`FoodStream.encoded` / `encoded_delta`). The version is recorded only after a successful send, so a raise that `_emit` swallows retries next tick, and because food is no longer in `state`, withholding a join-window frame cannot desync a client's pellets. Area-of-interest culling and protobuf stay deferred.
...
- **Own pieces are predicted; everyone else is interpolated.** Source plan section 6 calls interpolation non-optional so 30Hz state does not stutter at 60fps. That blend is always between two already-received snapshots, so off-LAN the body you steer sat a full RTT behind the mouse. `game.js` now dead-reckons *your* pieces from the latest snapshot plus the last sent `(dx, dy)` at `speed_for_mass`, decaying the error when the next snapshot arrives, and still interpolates every other player. The blend window is one nominal `1/tickRate` tick, then the latest snapshot is held — blending over a hitch-sized arrival gap left bots stuck on the previous frame, and extrapolating past the latest one drew ghost shadows on a laggy lobby. Self prediction is capped to about one tick ahead for the same reason. After a hitch the leftover prediction error is dropped rather than drawn as a second copy of your blob, and a split cluster is predicted at one shared velocity so different masses do not drift into two halves around the cursor. Kick velocity is not on the wire, so a split/merge/eat snaps instead of faking the pop. Input is sent every animation frame when the unit vector changes, not throttled to 20Hz. WebSocket stays; UDP/WebRTC is still deferred.
...
- [x] **[Agent]** `server/config.py`.
  - **From source plan section 5:** `TICK_RATE = 30`, `MIN_SPLIT_MASS = 35`, `MAX_PIECES = 8`, `EAT_RATIO = 1.25`. Live `SPLIT_KICK_DECAY_SECONDS = 0.6` (plan said ~0.5s). Plus `REMERGE_SECONDS = 10`, our pick from the plan's 10–15s range.
...
- [x] **[Agent]** `server/simulation.py` — `step(world, dt)`, which mutates the world in place and holds no module state, so a given `(world, dt)` always produces the same result. In this order:
  1. Apply each player's `last_input` as a normalized direction scaled by `speed_for_mass`. A one-piece player uses `speed_for_mass(piece.mass)`; a split cluster uses `speed_for_mass(total)` for every piece so light fragments cannot outrun the core. Plus the decaying split kick, and integrate position. Non-finite input is dropped: NaN compares false against every threshold below, so one bad value would skip the separation bail and spread into whatever it touched.
  2. Cluster forces: draw a player's own pieces toward each other at `COHESION_SPEED`, or home merge-ready pieces on the cluster centroid at `MERGE_PULL_SPEED + MERGE_RECALL * distance` once their remerge timers have cleared, so a fragment that drifted off still returns. Skipped for any pair whose split kick is still active, so cohesion never fights the kick.
  3. Resolve collisions by mass-weighted position projection, then clamp each piece so its **disc** stays inside the world (`center` inset by `radius`). Own pieces settle at `OWN_PIECE_OVERLAP` depth; different players' pieces are solid, *unless* one can eat the other, in which case they are left free to interpenetrate — and a spawn-protected prey is not an eat, so that pair stays solid and the predator is shoved off. Inert shards are left out of this pass entirely (not solid). **The clamp wins over separation**: a pair crushed into a corner keeps residual overlap. Bounds are inviolable, overlap is cosmetic.
...
- [x] **[Both]** Split halves pop apart on the kick, then snap back into contact on their own. Mechanism: `test_split_halves_drift_back_into_contact`, `test_cohesion_does_not_eat_into_the_split_kick`, `test_a_steering_split_cluster_stays_compact`. Watch it: the pop has to read as a lunge and the return as a quick cluster, not a floaty line.
...
- [x] **[Agent]** Food leaves the `state` broadcast. A `food` message of integer `[x, y]` pairs is a full snapshot for a new or skipped socket, and `{add, remove}` when the socket is exactly one version behind. See Divergence. `test_food_is_sent_once_then_not_resent_while_unchanged`, `test_eating_a_pellet_resends_food`, `test_a_late_joiner_receives_the_current_food_field`, `test_a_failed_food_send_is_retried_without_advancing_the_cursor`, `test_state_frame_without_food_is_under_4kb`, `test_food_stream_sends_rounded_pairs_and_only_bumps_on_change`, `test_food_index_apply_delta_skips_rebuild`.
...
- [x] **[Human]** Play against them in a browser tab. Confirm bots don't deadlock, don't spin in place, and don't crash on player disconnect.
...
- [x] **[Human]** World feels alive with bots present. Bots survive server restarts (the client reconnects, or its process exits cleanly).
- [x] **[Human]** Stress test: join as a **freshly spawning** player into a lobby with **≥17 bots**. The world feels alive (bots moving, eating, hunting around you). Your own movement, eating, and the spawn-invuln window stay responsive — not hitching, not a frozen or empty-feeling map.
...
- [x] **[Agent]** Write a short `scripts/vm_bootstrap.sh` — installs Python 3.13 (or 3.12), `pip`, sets up a venv, `pip install -r requirements.txt`, ufw rule. Also installs a `blobby.service` systemd unit. `scripts/tunnel.sh` is the CGNAT fallback (Cloudflare quick tunnel; no account).
...
- [ ] **[Human]** Start the server on the VM. After bootstrap: `sudo systemctl start blobby` (survives SSH disconnect). `tmux`/`screen` is fine instead.
